...
URL="${DL_UPDATE_URL:-https://get.joindreamlabs.com}"
...
FILES="server/run-agent.sh server/agent-jail.sh server/api-call.mjs server/dashboard.mjs server/connections.mjs server/dl-backend.mjs server/connectors/engine.mjs server/connectors/registry.mjs server/connectors/registry-generated.mjs server/briefing.mjs server/update-self.sh VERSION"
for rel in $FILES; do
...
node --check "$TMP/dl-backend.mjs" || { log "dl-backend.mjs failed parse"; status "error" "Downloaded dl-backend failed validation - no changes applied." ""; exit 1; }
node --check "$TMP/engine.mjs"    || { log "engine.mjs failed parse"; status "error" "Downloaded connector engine failed validation - no changes applied." ""; exit 1; }
node --check "$TMP/registry.mjs"  || { log "registry.mjs failed parse"; status "error" "Downloaded connector registry failed validation - no changes applied." ""; exit 1; }
node --check "$TMP/registry-generated.mjs" || { log "registry-generated.mjs failed parse"; status "error" "Downloaded connector registry failed validation - no changes applied." ""; exit 1; }
...
install -m 0644 "$TMP/dl-backend.mjs" "$DL_APP/bin/dl-backend.mjs"
mkdir -p "$DL_APP/bin/connectors"
install -m 0644 "$TMP/engine.mjs"    "$DL_APP/bin/connectors/engine.mjs"
install -m 0644 "$TMP/registry.mjs"  "$DL_APP/bin/connectors/registry.mjs"
install -m 0644 "$TMP/registry-generated.mjs" "$DL_APP/bin/connectors/registry-generated.mjs"
